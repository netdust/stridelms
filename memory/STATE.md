...
- **Webinar flow — Stride runs everything AROUND an external streaming platform (scoped 2026-07-03, after losing a client to WebinarGeek ~€69/mo).** Positioning ruling by Stefan: Stride will NEVER stream video; it owns discovery, flexible enrollment, payments, reminders, attendance, completion — the streaming tool (WebinarGeek/Zoom/YouTube/…) is pluggable. Grounded capability map (verified against source 2026-07-03): `webinar` is already a first-class `SessionType` enum case wired end-to-end — `webinar_link` URL field on `vad_session` (`SessionCPT.php:76`), admin metabox type picker, `requiresAttendanceMarking()` true so attendance→completion→LearnDash `markComplete`→certificate chain works today unchanged; `webinar` format slug already routes to `/online` catalog (`EditionService::ONLINE_FORMAT_SLUGS`).
  - *Missing (build later):* (1) **join-link email plumbing — the one real gap**: no `session.*` SmartCode group in `StrideMailBridge` and no session-date-triggered reminder ("your webinar is tomorrow, here's your link") — `GateReminderService` fires only on task deadlines; both cheap, infra exists (SmartCode registry + daily-cron reminder pattern). (2) Seat-type pricing (live seat vs. stream seat): today only member/non-member base price + per-session `price_modifier` (flows into quote line items) — workable day-one via two selectable slots with different modifiers; native ticket-variant concept is the only real modeling work. (3) Nice-to-haves: provider metadata (meeting ID/passcode vs. one free URL), auto-attendance import from the platform webhook (removes the one manual admin step).
  - *Competitive context:* WebinarGeek (NL, €49–349/mo) = webinar delivery + thin training features (assessments, Mollie/Stripe paid tickets) but NO courses/learning paths/native certificates/blended/company scoping — full analysis in session 2026-07-03. Qualify prospects on that fault line.
  - *Delivery integration ruling (Stefan, 2026-07-03): Cloudflare Stream Live as a pluggable provider — NOT hardcoded.* Mirror the `PaymentGatewayInterface` shape: `StreamingProviderInterface` contract in stride-core (`createLiveInput()` → RTMPS ingest creds, `getPlaybackToken(registration)` → signed embed JWT, `getRecording()` → replay), Cloudflare implementation in capability/client plugin, no binding = feature off; `webinar_link` stays as dumb fallback for Zoom/YouTube clients. Verified vs Cloudflare docs 2026-07-03: live inputs via API (RTMPS/SRT), auto-recording playable ~60s after end (free replay flow), signed playback tokens per-video via server-side signing keys with custom expiry, work for live+recordings → **per-registration access-controlled streaming** (WebinarGeek charges Enterprise €349/mo for this). Cost model: BYO Cloudflare account per client ($1/1000 min delivered + $5/1000 min stored ≈ $9 per 100-viewer 90-min webinar, billed Cloudflare→client directly) — Stride's ~€400/mo basic tier untouched; price integration like payments (first client pays wrapper, later clients provider-config only). Gaps: presenter needs OBS/StreamYard (no browser studio; CF WHIP ingest possible later, don't promise), auto-attendance via per-token player heartbeats = phase 2 (closes the manual marking step → completion → certificate). Threat model required at build (signed tokens = access boundary, CF API token = BYOK).
- **Pricing tiers — enforce in config + publish on stridelms.be (ruled 2026-07-03, build later).** Full strategy in auto-memory `project_pricing_tiers.md`. Tier ladder: **Basic ~€400/mo** = e-learning, webinars, on-demand paid courses (B2C checkout), vouchers, certificates (the WebinarGeek-gap hook — never fence out of Basic), course evaluations, admin workspace · **Pro** (upgrade trigger: "training leaves the browser") adds blended/in-person + attendance, trajectories, gates/approval + intake questionnaires, quotes/B2B invoicing, memberships & member pricing, accreditation-hours + annual reports, mail broadcast · **Enterprise** (trigger: other systems need access; priced for integration/SLA burden) adds Partner API (5 deferred bugs), LTI (parked), CRM sync, SSO, streaming provider, SLA. Second axis: per-active-user bands (Totara buyers expect it). Two build items: (1) **tier fencing enforceable in stride-core config** — per-client mu-plugin + FLAG default-off pattern, tiers = config not engineering; needs a decision on the flag surface (per-module service registration vs capability checks); (2) **stridelms.be pricing page** — marketing site location unknown, needs discovery first. Prereq reminder: Basic's paid on-demand requires the payment-gateway wrapper (above) + webinar join-link mail.
