...
**Tech Stack:** WordPress (Bedrock) · NTDST Core (DI, repositories, `ntdst/v1/action` registry) · `stride-core` mu-plugin · Alpine.js + the existing 955-line `admin-dashboard.js` · `wp_vad_registrations` custom table (already indexed — `idx_user`, `idx_edition`, `idx_trajectory`, `idx_parent`, `idx_status`, `idx_edition_status`, `idx_trajectory_status`, `idx_company`, `idx_user_status`, `idx_user_edition`; verified `RegistrationTable.php:70-79`) · `QuoteRepository` · DOMPDF/exporters.
...
| Traject | `trajectory_id` → trajectory title (when the row belongs to a trajectory; batch-resolved). Most rows are not in a trajectory → blank. |
...
- **Group-by is the control "Indelen per"** (Editie / Status / Organisatie) — labelled so it reads as restructuring the view, not sorting. It collapses the list into **collapsible sections** per distinct value; each section header shows aggregates **computed over the WHOLE group (not just the visible page)**: `count`, `% afgerond` (via `completed_at`), gem. aanwezigheid % (via `vad_attendance`), offerte-verdeling (a small quote-status breakdown, e.g. "2 verwerkt · 3 verzonden · 1 geen"). The first three are pure structured-column `GROUP BY` on `vad_registrations`; **the offerte-verdeling is a bounded postmeta join, NOT a structured-column aggregate** — named explicitly in §3.2 so it isn't mistaken for one. **Never** a questionnaire/`enrollment_data` JSON aggregate. This in-grid grouping replaces Excel pivot tables. Multi-select + bulk bar work inside grouped view.
...
- **Filter by trajectory** — just like the edition filter, the grid can be scoped to a trajectory (`trajectory_id` is indexed identically — `idx_trajectory`/`idx_trajectory_status`, verified). **Semantics (decided):** filtering by trajectory T shows the **child edition-registrations** of that trajectory (the actual course participations), NOT the parent row — so the grid stays edition-grained and attendance %/offerte/status columns keep their meaning per row. This requires the parent→child join, not a naïve `WHERE trajectory_id = T` (a trajectory is **1 parent row** `trajectory_id=T, edition_id=NULL` **+ N child rows** `edition_id=E, parent_registration_id=parent, trajectory_id=NULL` on cascade — a naïve filter would catch only the parent and miss every course row). The repository already has `findEditionsByTrajectory()`; the grid query reuses that join shape (§3.1). A small "Traject: <naam>" filter pill shows the scope, fed by a trajectory typeahead (same lightweight-picker pattern as editions, §10).
...
- **Trajectory section (when the person is in a trajectory)** — for each trajectory the user is enrolled in, show: the trajectory title + status + mode; **progress** (afgerond / bezig / nog te doen — `completed_count` / `in_progress_count` / `total_required`); the **required courses** with per-course state (afgerond via the LMS adapter `isComplete`, bezig via an active edition registration, nog te volgen); and the **elective groups** with **what the user chose vs. the group requirement** ("Keuze A: 2 van 2 gekozen — *Module X, Module Y*"). This is the user's other half of the story (the grid shows only edition rows). **This is the one genuine backend gap** — `getUserDetail` returns *no* trajectory data today (verified). But the compute already exists and is used by the live user-facing views (`TrajectoryDashboardService::getProgressData`, `TrajectorySelection::getSelectedCourseIds`/`isGroupChosen`/`countChosenInGroup`; mirrored from `tab-trajecten.php` + `tab-voortgang.php`) — so this is **wiring, not new logic** (see §11 + Task 3.5). Read selections via `TrajectorySelection` per **INV-6b**, never the raw `selections` column.
...
### Surface 4 — "Trajecten" (trajectory overview tab — IN the first slice, read-only)
A dedicated tab (alongside edities / offertes / gebruikers), **mirroring the editions-tab pattern**. This is a *view that does not exist today* — but its **backing endpoints already do**: `GET /admin/trajectories` (list) and `GET /admin/trajectories/{id}` (detail with the course list + enrolled users) are already in `AdminAPIController` (verified). So Surface 4 is mostly a **frontend tab over existing endpoints**, not net-new backend.
- **List:** each trajectory with title, status (Concept/Open/Volzet/Afgesloten/Gearchiveerd), mode (cohort/self-paced), capacity, enrolled count, course count. Search + status filter, active-by-default scope (same posture as editions, §10).
...
- **Jump to the grid:** from a trajectory, "Toon inschrijvingen" opens Surface 2 filtered to that trajectory (child edition-rows, per the filter semantics above) — the trajectory tab is the overview, the grid is where you act.
- **Phase 1 = read-only overview + jump-to-grid.** Trajectory-specific *bulk actions* on the trajectory roster are deferred (Phase 2 cohort work) — the existing per-registration bulk actions already apply once you're in the grid.
...
- Group-by aggregates are computed **from structured columns via SQL `GROUP BY`** on the same indexed table (count, completed_at-based % afgerond, attendance %) — no projection needed for aggregation either. **One exception, named explicitly: the offerte-verdeling (quote-status distribution) is NOT a pure structured-column GROUP BY** — `quote.status` lives in `vad_quote` postmeta, not on `vad_registrations`, so it requires the same two-step resolver as the offerte column (§0 #5): resolve the group's registration IDs → quote IDs (`findQuoteIdsByRegistrations`, a `postmeta` join with `MIN(p.ID)` GROUP BY, `QuoteRepository.php:135`) → one batched status read → tally. This is a **bounded** join (the group's registration-ID set, not the corpus) and stays off any JSON column (M5 intact), but it IS a postmeta join — do not let the "structured-only aggregate" framing hide it at review.
...
| `GET /admin/users/{id}/detail` | powers the case view's edition/quote/attendance sections (Surface 3) | **edition-only** — returns NO trajectory data (verified); the trajectory section needs the new `/users/{id}/trajectories` (above) |
| `GET /admin/users/{id}/reveal` (audited PII) | case-view PII reveal | none — reuse audited reveal posture |
| `GET /admin/trajectories` + `/admin/trajectories/{id}` | **the Trajecten tab (Surface 4)** — list + detail (courses + enrolled users) | none for the read-only overview; detail lacks per-user progress/electives (that's the case-view job, via `/users/{id}/trajectories`) |
| `TrajectoryDashboardService::getProgressData` + `TrajectorySelection` (`getSelectedCourseIds`/`isGroupChosen`/`countChosenInGroup`) | compute behind `/users/{id}/trajectories` | none — already used by `tab-trajecten.php`/`tab-voortgang.php`; admin endpoint just wires it |
| `RegistrationRepository::findEditionsByTrajectory` / `findByTrajectory` / `countByTrajectoryIds` | grid trajectory filter (parent→child join) + Trajecten-tab counts | none — verified, indexed; the grid filter reuses the join shape |
...
- **The gap is interaction-model, not capability.** Backend + REST + domain actions already exist (25 routes, the `/action` registry, the exporters). The missing pieces are: a grid component, multi-select state, a bulk-action bar, group-by. All are additive to the existing 955-line `admin-dashboard.js`.
- **Endpoint reuse:** the existing 25 endpoints + the `X-WP-Nonce` API helper + the admin menu registration are all reusable as-is. A React rewrite throws that wiring away and re-pays it.
...
- **"Active" = not terminal AND not past:** `OfferingStatus NOT IN (completed, cancelled, archived)` AND **(`end_date >= today` OR the edition has no dates yet)**. Read through `getEffectiveStatus()` (INV-7), never raw stored status. ⚠️ **A dateless edition is active, not past** — `EditionService::isPastDates()` already returns `false` when both `end_date` and `start_date` are empty (verified, `EditionService.php:275-277` "no dates at all → not past"), so the effective-status path is already correct. The danger is the *SQL* default, not the status logic: see §10.7.
...
### 10.7 The sessionless-edition carve-out — the cutoff must NOT hide not-yet-scheduled editions (correctness)

> ✅ **FIXED in `main` (2026-06-14, commit `e2ace22b`).** The admin `getEditions` half of this bug is already merged — `getEditions` now permits a NULL `start_date` in its default scope (LEFT JOIN + `OR pm_start.meta_value IS NULL`, NULL-last ORDER BY), so sessionless editions show in the default editions list. This section is **retained as the rationale + the contract the workspace's NEW query method (`queryForGrid`, Phase 1A) must mirror** — it no longer describes pending work on `getEditions`; it is the spec the proven fix already satisfies and that the new code must match.

The cutoff exists to hide **finished** editions. A **sessionless edition** (one with no sessions, hence no `start_date`/`end_date`) is the *opposite* of finished — it is **not yet scheduled**, and it is the row that **collects the interest list** (`InterestShortcodes` renders "for editions without sessions", verified `InterestShortcodes.php:10`). Users still see these on the public site to join the interest list; the admin must see them for the same reason — and so the §9 **"Oude interesse"** queue (line 46) has anything to count. A cutoff that hides them is a correctness bug, not a perf trade-off.

- **Effective-status is already correct.** `isPastDates()` returns `false` when both dates are empty ("no dates at all → not past", `EditionService.php:275-277`). So any scope built on `getEffectiveStatus()` (INV-7) **already** keeps dateless editions in the active set. The grid + queue scopes MUST be built this way.
- **The bug was the SQL default — now fixed in `getEditions`, still a contract for new code.** The old `getEditions` (`AdminAPIController.php:~788`) defaulted to `pm_start.meta_value >= twoDaysAgo` via an INNER JOIN on the `start_date` meta. A dateless edition **has no `start_date` meta row**, so that predicate dropped it (NULL ≥ date is never true) — the editions tab silently hid sessionless editions. `e2ace22b` corrected it to a NULL-permitting predicate (`pm_start.meta_value >= twoDaysAgo OR pm_start.meta_value IS NULL`, LEFT JOIN, NULL-last ORDER BY). **Any NEW query method that mirrors this default — the §10.4 grid/queue scope, Task 1.2's `queryForGrid`, the `/editions/options` typeahead — MUST use the same NULL-permitting shape** (or derive the active set from `getEffectiveStatus()` rather than re-deriving "past" in SQL). `e2ace22b` is the reference implementation to copy, not re-invent.
- **No per-stage special-casing needed.** Because "dateless = active" is a property of the **edition**, the interest rows ride along for free once the edition predicate is correct. There is no carve-out on the *registration* stage — only the edition predicate is fixed. (This supersedes an earlier draft idea of scoping interest rows by `registered_at`; the edition-level fix is strictly simpler and more correct.)
- **Aging interest is the queue's job, not the cutoff's.** "Old" interest is surfaced and cleared via the §9 **"Oude interesse"** lane (`registered_at` older than N days → Bericht sturen / archiveren), NOT by hiding its edition. The edition stays visible; the *task* ages.

*(Folds into Task 1.2 / 1.4a: the active-scope predicate is `getEffectiveStatus`-derived or NULL-permitting, never an inner-join-on-start_date. Add a test: a sessionless edition's interest rows appear in the default grid + the "Oude interesse" count.)*

> ✅ **The PUBLIC catalog had the SAME bug — also FIXED in `main` (2026-06-14, commits `787c696e..31c72487`).** `helpers/catalog.php` excluded dateless editions for the identical root cause (`stridence_catalog_date_window_meta_query()` paired with `orderby=meta_value`/`meta_key=start_date`, forcing a `start_date` EXISTS clause). This stranded the interest mechanism: `stride_interest` is built *for* dateless editions, but no catalog path reached it. **Ruling (Stefan, 2026-06-14), now shipped:** dateless editions **appear** on the public catalog — klassikaal in a labelled **"Binnenkort — toon interesse"** group (after dated-soon, before dated-grace, **always page 1**) with an interest-CTA card variant gated on effective status `Announcement`; online dateless editions are always-on and render as normal enroll cards (no band). That frontend fix was its own harnessed branch (`fix/dateless-editions-catalog`, merged), **separate from this admin spec** — cross-referenced here only because both halves share the `start_date`-must-exist root cause and were fixed consistently (`e2ace22b` admin / `787c696e..31c72487` catalog).

---

## 11. The trajectory layer (trajecten) — surfacing the second spine

The grid is built around `edition_id`, but `trajectory_id` is an equal first-class FK on the same table and was initially overlooked. This section adds it. **Almost all the machinery already exists** (grounded 2026-06-13) — the work is admin-side wiring, not new domain logic.

### 11.1 The data model (verified — get the parent/child right)
...
**Consequence:** "all registrations for trajectory T" = parent + children. A naïve `WHERE trajectory_id = T` catches only the parent (and legacy pre-cascade rows) and misses the course rows. `RegistrationRepository::findEditionsByTrajectory(userId, trajectoryId)` already does the correct parent→child join (handles both cascade + legacy shapes) — reuse it.

### 11.2 Grid filter (decided: show child edition-rows)
Filtering the grid by trajectory T shows the **child edition-rows** (the course participations), keeping rows edition-grained so attendance %/offerte/status stay meaningful. `trajectory_id` is indexed (`idx_trajectory`, `idx_trajectory_status`) — the filter is as cheap as the edition filter. Implemented in `queryForGrid` by routing the trajectory filter through the parent→child join shape (not a bare column WHERE). Fed by a trajectory typeahead (`/admin/trajectories/options`, mirror of §10's edition picker).

### 11.3 Trajecten tab (Surface 4 — read-only Phase 1, endpoints already exist)
...
`getUserDetail` returns **zero** trajectory data today — this is the only genuine gap. The fix is a new `GET /admin/users/{id}/trajectories` (keeps `getUserDetail` lean) that, for each trajectory the user is in, returns the progress + chosen-vs-required shape the **live user-facing views already render** (`tab-trajecten.php`, `tab-voortgang.php`). It wires existing compute, no new logic:
- `TrajectoryDashboardService::getProgressData(userId, trajectoryId)` → `required_courses`, `elective_groups`, `total_required`, `mode`, `edition_registrations`, plus BOTH the scalar progress counts (`completed_count`, `in_progress_count`) AND the course arrays (`completed_courses`, `in_progress_courses`) — verified return keys, `TrajectoryDashboardService.php:117-125`. The Dossier progress badge reads the `*_count` scalars (afgerond / bezig / `total_required`); the per-course state lists read the `*_courses` arrays. Use the exact keys — do not assume only the arrays exist.
- `TrajectorySelection::getSelectedCourseIds(registrationId)` / `isGroupChosen()` / `countChosenInGroup()` → what the user **chose** per elective group vs. the group requirement.
...
> ⚠️ Memory note: `TrajectorySelection::validateSelections()` has known field-shape bugs (`bug_trajectory_validate_selections_drift`) — but the case view uses the **read** methods above (`getSelectedCourseIds`/`isGroupChosen`/`countChosenInGroup`), which are the same ones the live frontend uses, NOT `validateSelections`. No exposure here; do not "fix" validateSelections as part of this work.

### 11.5 Net effect
Trajectory becomes the grid's second filter axis (cheap, indexed, child-row semantics), gets a read-only overview tab over existing endpoints, and completes the case view via one new wiring endpoint. *(Adds Tasks 1.4b — trajectory filter in `queryForGrid` + `/trajectories/options`; 3.5 — `/users/{id}/trajectories` + the case-view section; 3.6 — the Trajecten tab UI.)*
...
`AdminAPIController` is a **3,627-line, 150-`$wpdb`-call, 25-route god class** (verified 2026-06-13: `wc -l` = 3,627; `grep -c '$wpdb'` = 150; 25 `register_rest_route` calls). Every new endpoint this spec adds lands in it — so building the workspace on it *as-is* would deepen the very drift INV-3 forbids. Decision (user, 2026-06-13): **strangle as we go** — no big-bang refactor phase; **each build phase extracts ONLY the controller slice it touches** into a thin-controller + service layer, behavior-preserving, with the existing test suite as the green-before-and-after safety net. The fat methods (`getStats` 32, `getActionQueue` 18, `getUserDetail` 11, quotes 10) extract during the phases that already consume them; the bulk-action targets are already thin (0 `$wpdb`), so that phase moves no SQL. **The phase-by-phase extraction assignment lives in the companion `docs/plans/2026-06-13-admin-workspace-phase-map.md` (§1 seams, §4 strangle ledger).** By end of the first slice ~70 of 150 `$wpdb` calls have drained out, with no dedicated rewrite phase.

---

## 13. Edition-lifecycle task engine (Phase 2 — deferred, recorded here)

The §9 Vandaag queues are *row-state* lanes ("this registration is waiting"). This section adds **edition-lifecycle** tasks: work raised when an **edition** crosses a stage boundary. The spine is one insight — **interest and waitlist are holding pools that must flow forward when the edition they're attached to moves on**, otherwise an interest list dies with the placeholder edition that collected it (the §10.7 problem, but on the *forward* side: §10.7 keeps the pool *visible*; this section keeps it *actionable* across the transition).

**This is deferred to Phase 2.** It is recorded now so the idea is scoped, not lost. Phase 1 ships only the §10.7 cutoff correctness fix.

### 13.1 The transitions and the tasks they raise (decided 2026-06-13)

| Edition transition (detected) | Vandaag task raised | Admin action |
|---|---|---|
| Edition **ended** (`isPast` true / effective `Completed`) AND it has a non-empty waitlist | "Wachtlijst van [editie] afhandelen — verplaats naar interesse voor een volgende editie" | Review → move waitlist people onto the interest list for a future edition (or close) |
| A **new edition planned** for a course that has standing interest on a prior/sessionless edition | "[N] mensen toonden interesse in [cursus] — nodig hen uit voor de nieuwe editie" | Review → reach out / invite the interest list to the new edition |

These are seeds, not the closed set — the structure (edition transition → task) generalizes; more lanes can be added when concrete needs surface.

### 13.2 Detection — lazy, on dashboard load (decided 2026-06-13)

No cron. Tasks are **computed when Vandaag loads**, from edition dates + statuses already in the DB — consistent with the trajectory feature's existing "lazy task creation on dashboard load, no cron" decision ([[project_trajectory_phased_choices]]). An edition that ended yesterday surfaces its waitlist-handoff task the next time an admin opens the dashboard. "New edition planned" is detectable on the same pass (a course with a future/dateless edition AND prior interest rows with no outreach recorded).

### 13.3 Carry-forward — surface a task, admin acts (decided 2026-06-13)

Interest carry-forward is **human-in-the-loop, not automatic**. Planning a new edition does **not** silently re-point or copy interest rows (that would move PII without review and wrongly assume one edition is "the" continuation when a course can have parallel editions). Instead the task surfaces; the admin triggers the outreach. The interest rows stay where they are until acted on.

> **Post-launch alternative (noted, not chosen):** model interest on the **course** rather than the edition, so any new edition automatically "sees" the standing interest list. Cleaner long-term but a data-model change — out of scope here; revisit if parallel-edition outreach becomes routine. (Related: [[project_form_on_course_deferred]] — the same edition-vs-course tension on the form-schema side.)

### 13.4 Net effect & scope guard

Phase 2 adds: a detection pass on dashboard load (edition dates/status already present), two new Vandaag lanes, and a "mark outreach done" affordance so a handled transition stops re-surfacing. **No new infrastructure, no cron, no auto-migration.** Phase 1 builds **none** of this — only the §10.7 fix, which is the prerequisite (the interest pool must be *visible* before a task can ask you to *act* on it).
...
- [ ] Deviations from the slice (each named + justified):
      - **The read-model endpoint (`GET /admin/registrations`) is a REST route on `AdminAPIController`, not an `ntdst/api_data` filter** — justified: it is an admin-only read consumed by `X-WP-Nonce` fetch, consistent with the other 25 admin routes; the `api_data` path is for the *frontend* public-ish action surface. Authz via `canViewAdmin` (INV-1).
...
| **F7** *(trajectory)* | Trajecten tab → detail → jump to grid filtered by trajectory | Open Trajecten tab → pick a trajectory → see its courses + enrolled users → "Toon inschrijvingen" → grid filtered to that trajectory's **child edition-rows** (§11.2) | **empty:** a trajectory with 0 enrollments → "Nog geen inschrijvingen" in detail; jump-to-grid shows empty grid with the Traject pill set. **denied:** unauth → `/admin/trajectories` 401/403 (M1). **wrong-order:** jump to grid for a trajectory then clear the pill → grid widens back to all. **concurrent:** an enrollment changes while the detail is open → reflected on reopen. **boundary:** a trajectory with many child editions → grid pages them (server-side, §11.2). **mid-flow:** `/admin/trajectories/{id}` errors → "kon traject niet laden", tab list intact. **leak-check:** the filtered grid shows ONLY this trajectory's child rows, never another trajectory's (the join-correctness assertion, A2/Task 1.4b). |
| **F8** *(trajectory case-view)* | Open the Dossier of a trajectory-enrolled user → trajectory section | Click a trajectory user's row → Dossier → trajectory section shows progress (afgerond/bezig/te doen) + required courses with state + elective groups with chosen-vs-required | **empty:** user in NO trajectory → trajectory section absent (not an error); user in a trajectory with no electives chosen yet → "Nog geen keuzes gemaakt". **denied:** unauth → `/users/{id}/trajectories` 401/403 (M1). **wrong-order:** open a user whose trajectory is cancelled → shows the cancelled trajectory state, no crash. **concurrent:** a course completes while open → reflected on reopen. **boundary:** a user in multiple trajectories → each rendered as its own block. **mid-flow:** `/users/{id}/trajectories` errors → "kon traject-voortgang niet laden", rest of the Dossier intact. |
...
|---|---|---|
| **Phase 1 — Person workbench + trajectory layer (FIRST SLICE)** | Worklist home (Vandaag, 5 queues) + Inschrijvingen grid (multi-select, bulk-bar top-3 + overflow, group-by, server-side paging, **edition + trajectory filters**) + one Dossier case view (**incl. the trajectory progress section**) + the **Trajecten tab** (read-only overview + jump-to-grid) + the read-model endpoint + the ~9 bulk smart actions + `bulk_set_field` + the two lightweight picker endpoints (`/editions/options`, `/trajectories/options`) + `/users/{id}/trajectories` | **YES** |
| **Phase 2 — Cohort lens + edition-transition tasks** | Editie → sessie → per-session roster, attendance marking per session, roster extras columns/badges (per-edition loaded set), roster bulk actions, **trajectory-roster bulk actions**, existing exporters surfaced here, **+ the edition-lifecycle task engine (§13)** — Vandaag tasks raised when an edition crosses a stage boundary (ended → handle waitlist; new edition planned → invite past interest list) | No — deferred, cohort in §1, transition tasks in §13 |
...
**Do NOT over-scope Phase 1.** The cohort roster, trajectory-*roster bulk actions*, and field-scoped export are explicitly later phases. Phase 1's trajectory work is **read/filter only** — the layer is *surfaced* (filter + tab + case-view section), not made bulk-actionable on the trajectory roster itself.
...
- Tier A. **Test contract:** asserts the new `queryForGrid(array $filters): array` returns only rows matching a structured `status`+`company_id` filter, respects `per_page`/`page`, and that an **out-of-whitelist `sort`/`group_by` value is rejected** (M4) and **no param path reaches `enrollment_data`** (M5 — assert the method signature/whitelist, drive a JSON-ish param and confirm it's ignored/rejected); **asserts the active-edition scope default** (§10.4) — with `edition_scope=active` (default), rows on a terminal/past edition are excluded; with `edition_scope=all` or an explicit `edition_id`, they are included; **asserts the sessionless carve-out (§10.7)** — an interest row on a **dateless** edition appears in the default (`active`) scope, NOT hidden. Uses the integration DB (real indexes).
- [ ] Write failing test; run (FAIL); implement `queryForGrid` with server-side whitelists + `$wpdb->prepare` + the active-edition scope (active = `getEffectiveStatus` not terminal AND (`end_date >= today` **OR no dates yet**), default on; bypassed by explicit `edition_id` or `scope=all`). **The active-edition predicate is `getEffectiveStatus`-derived or NULL-permitting — NOT an inner-join-on-`start_date` (§10.7), which would drop dateless editions. Mirror the already-merged reference fix in `getEditions` (`e2ace22b`: LEFT JOIN + `OR pm_start.meta_value IS NULL` + NULL-last ORDER BY) — copy that proven predicate, do not re-derive it.** Run (PASS); commit.
...
#### Task 1.4b: Trajectory filter in `queryForGrid` (parent→child join) + `GET /admin/trajectories/options` (§11)
...
**Integration gate (cluster C1 — grid/worklist/case-view UI):** Playwright/Chrome acceptance pass F1–F6 against the running admin (per `feature-acceptance` situation B) — emit the pass/fail/not-reachable manifest. No UI flow is "pass" without the browser driving it. Confirm the dossier hides ≥1 empty stage, opens a stage on click, shows no duplicate Vragenlijst block, and the timeline renders a session-selection + an attendance event.

### ── REVIEW GATE C2 ── (tier: STANDARD — trajectory case-view endpoint + Trajecten tab; a new read endpoint + UI over existing endpoints. NOTE: the trajectory grid filter join lives in cluster A2, already FULL-reviewed.)

> Cluster C split (1f): C1 = the edition-centric UI (3.1–3.4a, STANDARD); C2 = the trajectory surfaces (3.5, 3.6, STANDARD). Keeps each review unit ≤4 tasks and isolates the trajectory layer.

#### Task 3.5: `GET /admin/users/{id}/trajectories` + the case-view trajectory section (§11.4)
...
- Tier B. `no unit test: Tier B, presentational tab over the already-tested /admin/trajectories(+/{id}) endpoints — F7 acceptance covers the flow`.
- [ ] Add the Trajecten tab: list (status/capacity/enrolled/courseCount, search + active-default scope), detail slide-over (courses + enrolled users), and "Toon inschrijvingen" → opens the grid filtered by `trajectory_id` (child-row semantics, §11.2); commit.

**Integration gate (cluster C2):** acceptance pass F7 (trajectory tab → detail → jump-to-grid) + the case-view trajectory section drives for a trajectory-enrolled user; manual `curl … /admin/users/<id>/trajectories` returns progress + chosen electives for an enrolled user, `[]` for a non-enrolled one, 401/403 anon.
...
- **`## Sibling-site audit` — the trajectory parent/child join.** "Registrations for trajectory T = parent + children" lives in: the grid trajectory filter (Task 1.4b), the Trajecten-tab jump-to-grid (Task 3.6), and (if reused) any count. They MUST all route through `findEditionsByTrajectory` / the verified join — a bare `WHERE trajectory_id = T` anywhere is a bug (misses child rows) AND a leak risk if it crosses trajectories. Sweep every trajectory-scoped query against the join shape (§11.1).
- **`## Sibling-site audit` — trajectory selection READ vs validate.** The case view + any trajectory progress read MUST use `TrajectorySelection::getSelectedCourseIds`/`isGroupChosen`/`countChosenInGroup` (the methods the live frontend uses), NEVER `validateSelections` (known-buggy, `bug_trajectory_validate_selections_drift`). If a reviewer sees `validateSelections` in admin-workspace code, that's a finding.
