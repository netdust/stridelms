...
- **The real bug is the existing SQL default.** `getEditions` (`AdminAPIController.php:~788`) defaults to `pm_start.meta_value >= twoDaysAgo` via a JOIN on the `start_date` meta. A dateless edition **has no `start_date` meta row**, so that predicate drops it (NULL ≥ date is never true) — the editions tab **already silently hides sessionless editions today**. Any query method that mirrors this default (the §10.4 grid/queue scope, Task 1.2's `queryForGrid`, the `/editions/options` typeahead) MUST use a NULL-permitting predicate so dateless editions survive: `pm_start.meta_value >= twoDaysAgo OR pm_start.meta_value IS NULL` (LEFT JOIN, not INNER) — equivalently, derive the active set from `getEffectiveStatus()` rather than re-deriving "past" in SQL.
...
> **The PUBLIC catalog has the SAME bug — ruled & tracked separately.** `helpers/catalog.php` excludes dateless editions for the identical root cause (`stridence_catalog_date_window_meta_query()` pairs with `orderby=meta_value`/`meta_key=start_date`, forcing a `start_date` EXISTS clause — runtime-proven). This stranded the interest mechanism: `stride_interest` is built *for* dateless editions, but no catalog path reached it. **Ruling (Stefan, 2026-06-14):** dateless editions **appear** on the public catalog, in a labelled **"Binnenkort — toon interesse"** group placed after dated-soon and before dated-further-out (mid-page), **always on page 1**. That frontend fix is its own harnessed change — tracked at `tasks/todo.md` item 3 — NOT part of this admin spec; it is cross-referenced here only because admin §10.7 and the catalog fix share the `start_date`-must-exist root cause and must not be fixed inconsistently.
...
- [ ] Write failing test; run (FAIL); implement `queryForGrid` with server-side whitelists + `$wpdb->prepare` + the active-edition scope (active = `getEffectiveStatus` not terminal AND (`end_date >= today` **OR no dates yet**), default on; bypassed by explicit `edition_id` or `scope=all`). **The active-edition predicate is `getEffectiveStatus`-derived or NULL-permitting — NOT an inner-join-on-`start_date` (§10.7), which would drop dateless editions.** Run (PASS); commit.
