...
> 1b. **[cross-plugin, hand to mail-broadcast]** `SmartCodeParser::parse()` in netdust-mail returns smartcode values with NO `esc_html()` — a crafted user display name/edition title lands unescaped in mail bodies. Systemic to all 18 templates (pre-existing, this feature didn't worsen it). Low-impact for email but a real hardening ticket; fix at the `SmartCodeParser` seam alongside the `is_email` hardening (#1) when mail-broadcast is built. ([[project_mail_broadcast_feature]])
