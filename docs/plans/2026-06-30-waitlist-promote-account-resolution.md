...
- [ ] **Step 1:** Configure the `waitlist`-stage groups to include the chosen field set, using the exact reserved INPUT-KEYS from `getUserMetaMapping()` as field names (`company, address, postal_code, city, vat_number, invoice_email, gln_number, organisation, department`) so they auto-map on promote. NOT the `billing_*` meta keys.
- [ ] **Step 2: Confirm `sanitizeExtraFields` covers the new field types** (text/email) and `QuestionnaireValidator` accepts the stage schema — no structural template/handler change; if a type isn't covered, that's the only code edit here.
- [ ] **Step 3: Round-trip proof** — the Task 3 integration case (a) submits these exact fields and asserts they land on the new user's usermeta. Run it; confirm GREEN with the real configured field names.
- [ ] **Step 4: Commit** `feat(waitlist): collect offer/invoice field set upfront on the waitlist stage`.

`no unit test for the form render: Tier B, reused field-group component + validator. The load-bearing assertion is the Task 3 round-trip (M-ROUNDTRIP).`
