...
- **Online-sessie datum ⇄ LearnDash drip sync (admin-keuze)** — today the online-session date (Stride) and the lesson availability/drip date (LearnDash) are two independent fields an admin must keep in sync by hand: the session date shows "Beschikbaar vanaf …" and nudges the dashboard but does NOT lock the lesson; only LearnDash drip enforces access (verified 2026-06-29). Future: an **opt-in admin choice** on the online session — "vergrendel deze les tot de sessiedatum" — that, when ticked, writes the session date into the lesson's LearnDash `visible_after_specific_date` so a single input both displays and enforces. Must stay a choice (not automatic): some blended setups want the date as a soft nudge only, and writing course-level LD drip from an edition affects every edition of that course. Current two-field reality is documented in the handleiding (§ Blended & online sessies → "Sessiedatum vergrendelt niet").
