...
> **RESOLVED 2026-06-26 (Stefan):** Q1 → **(a)** keep legacy `edition.documents` on public URLs; gate only `vad_document`. Q3 → **`stride_view` too** (supervisors see the download log; follows the existing dossier gate). Q2 is a verify-at-implementation step for the implementer in Task 5.1, not a Stefan decision. Build to these.

1. **[RESOLVED → (a)] Legacy `edition.documents` download path.** The new gated endpoint applies to `vad_document` only. Today's `edition.documents` attachments are rendered via PUBLIC `wp_get_attachment_url()` in the metabox + dashboard — i.e. they are NOT gated. **Decision: (a)** — leave legacy edition docs on public URLs, surface them in the frontend merge as-is, gate only `vad_document`. Task 4.2's legacy-fold keeps public URLs; Mitigation 1's "one allowed exception" stands.
...
3. **[RESOLVED → `stride_view`] Download audit visibility (privacy).** The dossier download log is per-employee behavioural data. **Decision:** `stride_view` (supervisor) MAY see download timestamps — follows the existing dossier gate, matches the client's "track what employees followed" intent. Mitigation 10's cap = `stride_view` (NOT promoted to `stride_manage`).
