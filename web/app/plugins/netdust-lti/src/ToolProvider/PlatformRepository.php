 ...
 *
 * Stores platforms that connect TO us (external LMSs that launch our courses).
...
final class PlatformRepository
{
    ...
    public function findByIssuerAndClient(string $platformId, string $clientId): WP_Post|WP_Error
    {
        ...
        // Data Manager may return 'ID' or 'id' depending on version
        $postId = $results[0]['ID'] ?? $results[0]['id'] ?? 0;
        if (!$postId) {
            return new WP_Error('not_found', 'Platform not found');
        }
