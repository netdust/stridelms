		...
		if ( is_wp_error( $plugin_info ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Could not retrieve download information for "%1$s": %2$s', 'learndash' ),
		...
		if ( empty( $plugin_info->download_link ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'No download link is available for "%s".', 'learndash' ),
					$this->feature->get_name()
				)
			);
		}
	...
	protected function do_activate() {
			...
			static function () use ( $plugin_file, &$completed, &$die_output ) {
				...
				$error = new WP_Error(
					...
					sprintf(
						...
						__( 'The plugin "%s" called exit/die during activation and terminated the process.', 'learndash' ),
						$plugin_file
					),
					[ 'status' => 422 ]
				);
		...
		try {
			...
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: feature name */
					__( 'A fatal error occurred while activating "%s".', 'learndash' ),
					$this->feature->get_name()
				)
			);
		}
		...
		if ( is_wp_error( $result ) ) {
			...
			return new WP_Error(
				...
				sprintf(
					/* translators: %1$s: feature name, %2$s: error message */
					__( 'Activation of "%1$s" failed: %2$s', 'learndash' ),
					$this->feature->get_name(),
		...
		if ( ! $this->check_active() ) {
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: feature name */
					__( '"%s" did not activate successfully. Please try again.', 'learndash' ),
					$this->feature->get_name()
				)
			);
		}
	...
	protected function do_deactivate() {
		...
		if ( $this->check_active() ) {
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: feature name */
					__( '"%s" could not be deactivated. The plugin may have been reactivated by another process.', 'learndash' ),
					$this->feature->get_name()
				)
			);
		}
