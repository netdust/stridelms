...
use StellarWP\Learndash\LiquidWeb\Harbor\Licensing\Results\Product_Entry;
use StellarWP\Learndash\LiquidWeb\Harbor\Traits\With_Debugging;
use StellarWP\Learndash\LiquidWeb\Harbor\Traits\With_Error_Throttle;
...
class License_Manager {
	...
	public function validate_and_store( string $key, string $domain, bool $network = false ) {
		...
		if ( ! License_Key::is_valid_format( $key ) ) {
			...
			return new WP_Error(
				...
				__( 'The license key format is invalid.', 'learndash' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( ! $this->repository->store_key( $key, $network ) ) {
			...
			return new WP_Error(
				...
				__( 'The license key could not be stored.', 'learndash' ),
				[ 'status' => 500 ]
			);
		}
	...
	public function get_products( string $domain ) {
		...
		if ( $key === null ) {
			return new WP_Error(
				Error_Code::INVALID_KEY,
				__( 'No license key is stored.', 'learndash' ),
				[ 'status' => 422 ]
			);
		}
		...
		try {
			...
			return new WP_Error(
				...
				sprintf(
					...
					__( 'License key "%1$s": %2$s', 'learndash' ),
					$key,
					$e->getMessage()
				),
			...
			return new WP_Error( Error_Code::INVALID_RESPONSE, __( 'An unexpected error occurred.', 'learndash' ), [ 'status' => 500 ] );
		}
	}
