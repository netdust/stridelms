...
/**
 * Quote PDF Generator Helper
 *
 * Generates PDF documents for quotes using DOMPDF.
 * PDFs are stored securely and served through authenticated endpoints.
 *
 * This is a pure helper class - no WordPress hooks.
 * Route registration should be done by QuoteService.
 *
...
class QuotePDFGenerator
{
    ...
    {
        // Get user data via filter (allows SubscriberService to provide data)
        $userId = (int) ($quote['user_id'] ?? 0);
        $userData = apply_filters('stride/quote/get_user_data', [
            'name' => '',
            'email' => '',
        ], $userId);

        // Get course/item data via filter
        $itemType = $quote['item_type'] ?? 'course';
        $itemId = (int) ($quote['item_id'] ?? $quote['course_id'] ?? 0);
        $itemData = apply_filters('stride/quote/resolve_item', [
            'title' => '',
        ], $itemType, $itemId);
        ...
        $quote['company'] = [
            ...
            'country' => $this->getConfig('company_country', 'Belgium'),
    ...
    private function getConfig(string $key, mixed $default = null): mixed
    {
        ...
        // Check in company sub-array first
        if (isset($config['modules']['invoicing']['company'][$key])) {
            return $config['modules']['invoicing']['company'][$key];
        }
