...
test.describe('Trajectory Discovery & Navigation', () => {
  ...
  test('trajectory catalog displays trajectory cards', async ({ page }) => {
    ...
    await expect(page.locator('.stride-catalog__title')).toBeVisible();

    // Check for either trajectory cards or empty state
    const hasTrajectories = await page.locator('.stride-trajectory-card').count();
  ...
  test('trajectory card shows key information', async ({ page }) => {
    ...
    if (await firstCard.isVisible({ timeout: 1000 }).catch(() => false)) {
      ...
      await expect(firstCard.locator('.stride-trajectory-card__title')).toBeVisible();

      // Card should have stats (course count)
      await expect(firstCard.locator('.stride-trajectory-card__stats')).toBeVisible();

      // Card should have price or free label
      const hasPricing =
        (await firstCard.locator('.stride-trajectory-card__price').isVisible().catch(() => false)) ||
        (await firstCard.locator('.stride-trajectory-card__price--free').isVisible().catch(() => false));
  ...
  test('clicking trajectory card navigates to detail page', async ({ page }) => {
    ...
    if (await firstCard.isVisible({ timeout: 1000 }).catch(() => false)) {
      ...
      const trajectoryLink = firstCard.locator('.stride-trajectory-card__title a');
      const href = await trajectoryLink.getAttribute('href');

      await trajectoryLink.click();

      // Should navigate to trajectory detail page
  ...
  test('trajectory detail page has enrollment button', async ({ page }) => {
    ...
    const openCard = page.locator('.stride-trajectory-card:not(.stride-trajectory-card--closed)').first();
    if (await openCard.isVisible({ timeout: 1000 }).catch(() => false)) {
      // Navigate to detail page
      await openCard.locator('.stride-trajectory-card__title a').click();
...
test.describe('Authentication Gate', () => {
  test('unauthenticated user sees login prompt on enrollment page', async ({ page }) => {
    ...
    if (await confirmLink.isVisible({ timeout: 500 }).catch(() => false)) {
    ...
    const loginButton = page.locator('a[href*="wp-login"]');
  ...
  test('login with redirect returns to enrollment page', async ({ page }) => {
    ...
    const loginWithRedirect = `${urls.login}?redirect_to=${encodeURIComponent(enrollmentUrl)}`;

    await page.goto(loginWithRedirect);
...
test.describe('Already Enrolled Handling', () => {
  test('already enrolled user sees message instead of form', async ({ page }) => {
    ...
    // Navigate to enrollment page for a trajectory they're enrolled in
    // This assumes seed data has enrolled this user
    await page.goto('/inschrijven/?trajectory=1');

    // Check for already enrolled message OR form (if not enrolled in this specific trajectory)
    ...
    expect(hasAlreadyEnrolled || hasForm || hasNoItem).toBeTruthy();
  });
...
test.describe('Enrollment Closed Handling', () => {
  ...
  test('catalog indicates closed trajectories', async ({ page }) => {
    ...
    // Check for closed trajectory cards
    const closedCard = page.locator('.stride-trajectory-card--closed');
    const closedStatus = page.locator('text=Inschrijving gesloten');

    // If there are closed trajectories, they should be marked
...
test.describe('Mobile Responsive', () => {
  ...
  test('trajectory catalog cards stack on mobile', async ({ page }) => {
    ...
    const cards = page.locator('.stride-trajectory-card');
