...
2. **Registrations UNIQUE key**: add the missing UNIQUE constraint (user_id, edition_id, status-class) on `wp_vad_registrations` at the next SCHEMA_VERSION bump — piggyback on the version-gated migrate() (now with a per-step cursor + failure backoff), never as a standalone ALTER. *Deliberately NOT piggybacked on v5 (2026-07-14): (a) the ALTER fails on existing duplicate rows → needs a dedupe pass first; (b) a PLAIN (user_id, edition_id) key was already tried and DROPPED in June 2026 (`gotcha_bad_unique_user_edition_constraint` — it broke Cancelled-row reactivation and trajectory cascade children), so the design must be status-class-aware or it re-ships that breakage. Interim primitive: `acquireEnrollLock()` advisory lock. Full context: `docs/DATA-MODEL-REGISTRATIONS.md` §5.*
