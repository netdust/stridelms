...
- [ ] **Phase 3 — Targeted P0 edge-testing gate**: drive enrollment edges (capacity-full, double-submit, validation errors, colleague PII), attendance flows (zero acceptance coverage), completion→certificate, dashboard empty states, quote locking, anonymise — through real browser; new Cests as regression net; flip FEATURE-STATUS rows with evidence
- [ ] **Phase 4 — Deploy readiness**: deploy-time list + evidence-trail refresh. ⚠ `site.yml` declares `make deploy-staging` but **no Makefile exists** — deploy tooling must be created/verified before launch. Also: production .env must NOT set STRIDE_TEST_LOGIN_SECRET.
