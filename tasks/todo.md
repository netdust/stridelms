...
2. **Registrations UNIQUE key**: add the missing UNIQUE constraint (user_id, edition_id, status-class) on `wp_vad_registrations` at the next SCHEMA_VERSION bump — piggyback on the version-gated migrate() (now with failure backoff), never as a standalone ALTER. *Deliberately NOT piggybacked on v5 (2026-07-14, lead_name/lead_email): the ALTER fails on existing duplicate rows, so the constraint needs a dedupe/cleanup pass designed first — retarget to v6 WITH that pass, don't slip it into an unrelated bump.*
