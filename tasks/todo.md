...
3. **Dateless / self-paced catalog ruling** — ✅ RULED (Stefan, 2026-06-14): dateless editions **DO appear** in the catalog, in their own labelled **"Binnenkort — toon interesse"** group placed AFTER dated-soon editions and BEFORE dated-further-out (mid-page, not all-top/all-bottom), and **always on page 1** (the interest front door is never behind "Toon meer"). Card uses an interest variant ("geen datum — toon interesse", CTA to the interest flow). Fix is two-sided and shares a root cause with the admin cutoff bug (`start_date` must-exist): (a) **frontend** `stridence_catalog_date_window_meta_query()` + the `orderby=meta_value/meta_key=start_date` trap in `*_items()` (helpers/catalog.php) must stop forcing the EXISTS clause and instead order into the three-band layout; (b) **admin** `getEditions` (AdminAPIController.php:~788) per the Admin Workspace §10.7. NOT yet built — needs a harnessed plan (query semantics + ordering + paging interaction + card variant + tests). Also still covers the archive classroom teaser's missing date window (pre-existing divergence, noted inline).
