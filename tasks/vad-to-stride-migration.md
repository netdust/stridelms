...
- [ ] **After any DB import that predates the 2026-05-19 URL rework, run `wp option delete stride_url_slugs && wp rewrite flush`.** A stored `stride_url_slugs` option pins `edition => 'vormingen'` and overrides the new `'edities'` code default (`StrideSettingsService::DEFAULT_SLUGS`), silently reviving the old URL model (see `tasks/url-structure-rework.md`). Found 2026-06-12 on the local dev DB. Related gap: the rework's planned 301 redirects `/vormingen/...` → `/edities/...` were never implemented — decide before launch whether old indexed/emailed URLs need them.
