...
foreach ($enrollments as $enrollment) {
    ...
    if ($courseId) {
        $includedCourseIds[] = $courseId; // Track this course
    }
...
if (function_exists('learndash_user_get_enrolled_courses')) {
    ...
    foreach ($ldCourses as $courseId) {
        ...
        // Build course data for online course
        $courseData = [
            'edition_id' => null,
            'course_id' => $courseId,
            'title' => $courseTitle,
            'url' => get_permalink($courseId),
            'progress' => ['percentage' => $percentage, 'is_complete' => $isComplete],
            'is_complete' => $isComplete,
            'percentage' => $percentage,
            'total_sessions' => 0,
            'attended' => 0,
...
<div class="stride-my-courses stride-dashboard-courses stride-dashboard-page">
    <!-- Page Header -->
